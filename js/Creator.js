 ...
 *****************    THE MATRIX    *****************   
 ***************************************************/
var valuesLevel = ['Extremly Easy', 'Easy', 'Medium', 'Difficul', 'Evil'];
...
function createTable() {
  ...
  $('<br>').appendTo('#SudControl');
  var labelLevel = $('<div></div>', {
    id: 'labelLevel',
    text: 'Level: Medium'
  });
  labelLevel.appendTo('#SudControl');
  var slideLevel = $('<div></div>', {
    id: 'slideLevel',
  });
  slideLevel.appendTo('#SudControl');
  ...
  $('#slideLevel').slider({
    ...
    slide: function(event, ui) {
      $('#labelLevel').text('Level: '+valuesLevel[ui.value]);
    }
  });
  $('#labelLevel').val(valuesLevel[$('#slideLevel').slider('value')]);
...
function tableValidations() {
  for (var i = 1; i < 10; i++) {
    for (var j = 1; j < 10; j++) {
      ...
      $("#R" + i + "C" + j).keydown(function(event) {
        onlyNumbers(event)
      });
      $("#R" + i + "C" + j).keyup(function() {
        checkErrors()
      });
...
function createGrid() {
  ...
  $.ajax({
    type: 'POST',
    url: 'src/getGrid.php',
    data: { level: valuesLevel[$('#slideLevel').slider( "option", "value" )] },
    dataType: 'JSON',
    async: false
